...
class MvtecAdBinary(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    os.remove('./Extracted/readme.txt')
    os.remove('./Extracted/license.txt')
  ...
  def _generate_examples(self, path, tag):
    ...
    for label in tf.io.gfile.listdir(path):
        ...
        for image in tf.io.gfile.glob(str(label_folder)+'/*/*.png'):
            ...
            label_classnm = os.path.basename(image)
            if label_classnm == "good":
              label_class = 0
            else:
              label_class = 1
